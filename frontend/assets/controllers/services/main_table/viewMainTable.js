...
export function ViewMainTable(vmMainTable, formElement){
    ...
    this.tableErrorMessage = document.querySelector('.table-error-message');
    this.tableErrorButton = document.querySelector('.closeTable');

    document.querySelectorAll(".buttonCloseTable").forEach((btn) => {
        btn.addEventListener("click", () => {
            window.dispatchEvent(
                new CustomEvent('image:open', { detail: 'image' })
            );

            this.vmMainTable.destroyMainTable(this.formElem);
        });
    });
